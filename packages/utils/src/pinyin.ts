 ...
 * @internal
 */
export const pinyinCache: (string[] | null)[] = []

/**
 * 获取单个汉字的拼音
 * @param charCode 要处理的字符编码
 * @returns 返回字符的所有发音，如果不是汉字或无法识别则返回 `null`
 * @example getPinYin("重") // ["chong", "zhong"]
 */
export function getPinYinsOfChar(charCode: number) {
	const cached = pinyinCache[charCode]
	if (cached !== undefined) {
		return cached
	}
	let result: string[] | null = null
	for (const pinyin in dict) {
		const chars = dict[pinyin]
		for (let i = 0; i < chars.length; i++) {
			if (chars.charCodeAt(i) === charCode) {
				result ??= []
				result.push(pinyin)
				break
			}
	...
	return pinyinCache[charCode] = result
}
...
/**
 * 将汉字转为拼音，其中非汉字保留不变，多音字返回第一个发音
 * @param str 要处理的字符串
 * @param seperator 每个拼音的分隔符
 * @example toPinYin("重庆1990") // "chongqing1990"
 */
export function toPinYin(str: string, seperator = "") {
	return str.split("").map(char => {
		const pinyins = getPinYinsOfChar(char.charCodeAt(0))
		return pinyins ? pinyins[0] : char
	}).join(seperator)
}